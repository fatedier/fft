...
func init() {
	...
	rootCmd.PersistentFlags().StringVarP(&options.ServerAddr, "server-addr", "s", version.DefaultServerAddr(), "remote fft server address")
	rootCmd.PersistentFlags().StringVarP(&options.BindAddr, "bind-addr", "b", "0.0.0.0:7778", "bind address")
	rootCmd.PersistentFlags().StringVarP(&options.AdvicePublicIP, "advice-public-ip", "p", "", "fft worker's advice public ip")
	...
	rootCmd.PersistentFlags().IntVarP(&options.MaxTrafficMBPerDay, "max-traffic-per-day", "", 0, "max traffic fftw can use every day, 0 means no limit, unit is MB, default is 0MB and min value is 128MB")
