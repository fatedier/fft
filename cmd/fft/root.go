...
func init() {
	...
	rootCmd.PersistentFlags().StringVarP(&options.SendFile, "send-file", "l", "", "specify which file to send to another client")
	rootCmd.PersistentFlags().IntVarP(&options.FrameSize, "frame-size", "n", 5*1024, "each frame size, it's only for sender, default(5*1024 B)")
